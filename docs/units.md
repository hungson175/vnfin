...
| Indices | **points** | `client()` = multi-source failover (VPS → SSI → VNDirect); `source()` = VPS primary. NOT VND — index sources use scale 1.0; do not route through the ×1000 price sources. |
| Fundamentals | **raw VND** | `client()` = **VNDirect (primary) → CafeF (backup)**; `source()` = VNDirect primary. Unscaled (e.g. total assets in trillions of VND as integer VND). Income/balance/ratios have true redundancy; **cashflow is VNDirect-only** (CafeF summary handlers don't serve it). |
