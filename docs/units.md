...
| Equities (universe) | **none** (reference metadata, not a price) | **Single-source (SSI iBoard)** — `client()` is an alias of `source()`. `universe(exchange=...)` enumerates the investable equities per board (HOSE/HNX/UPCOM) with per-symbol reference metadata (`par_value` is VND when present; everything else is text/None). Index-basket-derived (~96% of the full SSC roster — `partial_universe_coverage` always present); no listing date / no sector (always-present warnings); not a screener/ranker. See [sources/equities-universe.md](sources/equities-universe.md). |
...
| Funds (NAV) | **VND / fund unit** (model contract) | **Disabled pending permission.** `source()` and `client()` remain the same lazy Fmarket adapter; every valid operation raises `SourceUnavailable("SOURCE_DISABLED_PENDING_PERMISSION")` before cache/network. No fallback or alternate source is used. |
