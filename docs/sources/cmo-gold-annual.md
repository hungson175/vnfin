# Source — World Bank CMO "Pink Sheet" annual gold

**Domain:** internal world-gold annual leg of `vnfin.gold.world_reference_history_vnd` (issue #185)
...
4. Walk the worksheet rows. The **year is in column 0** (a data row is one whose col-0 cell is a
   4-digit year). The **gold header is SPLIT across two cells** — there is **no single combined
   string** — so the gold column is matched by **BOTH** cells of its split header:
   - a name-row cell whose text is exactly `Gold` (trimmed), **and**
   - the units-row cell **directly below it, same column**, whose text is exactly `($/troy oz)`.

   The match requires both cells because sibling columns (e.g. Platinum) also carry
   `($/troy oz)`; the name disambiguates. The gold column is located **by this text match, never a
   hard-coded index** (real vintage: column index 67 / `BP`, but it shifts). Numeric cells (year,
   price) are stored as **raw numbers** (no `t="s"`, often no `t` attr); header cells are shared
   strings — both cell types are handled. The title/metadata rows above the header and the two
   header rows are skipped.

Any malformed/unexpected condition fails safe as `InvalidData`: missing `Gold` name cell, missing
or mismatched `($/troy oz)` units cell, sheet not found, no data rows, non-numeric price, or a
non-xlsx body (bad zip magic / truncated / HTML).
...
- **Integrity / magnitude guards:** reject non-finite / `<= 0` prices, duplicate / non-monotonic
  years, and (gate note **N1**) any value outside a plausible **20 … 10000 USD/oz** band — all as
  `InvalidData`. The band is generous (it only catches gross column misparses, never legitimate
...
- **Annual only** — CMO publishes annual (and monthly) Pink Sheet workbooks; this adapter reads the
  annual nominal-USD sheet only.
- **Internal source** — `WorldBankCmoGoldSource` is consumed only by `world_reference_history_vnd`
  (as the primary world-gold leg); it is not exported on the public API surface.
