...
**Scope:** source, legal, coverage, and API-boundary evidence only. No new #235 cell-audit provider
request or raw-row retention was made by this builder; no source registration, RED test, API/model
change, production code, push, or issue closure was performed.
...
research contains bounded USA examples for three annual cells, and the accepted packet carries a
one-year ranking observation for each frozen member's GDP. The remaining cells are not promoted from
a syntactically routable country code or a generic dataset page. They remain `NOT_PROBED`, with the
USA policy-rate cell at `SEMANTICS_GAP` because the current display contract names an SBV proxy. This
is a useful coverage inventory, not a claim that the five-country × ten-indicator panel is available.
...
The accepted packet's upstream ranking evidence is a separate retained evidence unit: reviewer
anchor `acbbb82`, path `docs/research/2026-08-31-issue235-north-america-macro-cohort.md`, blob
`621545ca505ff6234cad32b9be7989d5e5add426`, observed 2026-08-31. It binds the official WDI
`NY.GDP.MKTP.CD` multi-country 2020 query and the incomplete 2025 query, including returned
`countryiso3code`, `indicator.id`, `date`, `value`, and current-US-dollar semantics. This is
ranking-only response evidence; it is not a 50-cell series-audit dispatch.
...
- No post-intake #235 cell-audit provider dispatch was made by this builder. The upstream ranking
  unit above is the only additional response-backed evidence and is explicitly limited to one-year
  GDP ranking observations; every other row without an explicit retained response reference is
  `NOT_PROBED`, and a route declaration is not evidence of observations, bounds, or rights.
...
year. The accepted packet freezes **2020** as the cohort boundary. This packet does not independently
prove that no later common year exists: 2021–2024 were not independently retained or audited here,
and the official query is reproducible ranking context rather than a new latest-year claim:
...
| Route/method | Current adapter: `GET https://api.worldbank.org/v2/country/{ISO3}/indicator/{CODE}?format=json&per_page={N}&date={Y1}:{Y2}` for one country/one code; future exact batch route below adds `source=2` and binds the WDI source explicitly |
| Response identity | Future batch returned source identity must reconcile to request `source=2`, plus `countryiso3code`, `indicator.id`, `date`, `value`, `unit`; a syntactically valid ISO3 or code is not response identity |
| Transport | JSON envelope `[meta, observations]`; status, complete `Content-Type`, redirect final identity, and TLS outcome must be retained by any future probe; no post-intake #235 cell-audit dispatch occurred; upstream ranking evidence is separate |
...
The exact multi-country/multi-indicator route reserved for a future one-call observation is:
...
This binds `route_version=v2`, `method=GET`, and request query `source=2`; the returned source
identity must be retained exactly as exposed by the provider and reconcile to source `2` (missing,
ambiguous, or mismatched source identity fails closed). The expected response is one JSON envelope
`[metadata, observations]`. Retain the returned `page`, `pages`, `per_page`, `total`, and any documented
source identity; no value for those response fields is predeclared here except the requested
`per_page=20000`. The packet-derived `5 × 8 × 66 = 2,640` is only the maximum expected
country/indicator/year position envelope, not an asserted provider `total`. Reconcile the observed
total and page metadata with the provider's documented row/null semantics and every returned
`countryiso3code`, exact `indicator.id`, four-digit `date`, and provider `value`/`unit`. Missing/null
positions stay missing. A missing or unexpected identity, unresolvable observed total/page contract,
page mismatch, or late failure invalidates the whole observation. This route is bound as design
evidence, not a dispatch.
...
| Status/MIME/coverage | No post-intake #235 cell-audit response was dispatched or retained. Exact dimensions, row count, first/last period, nulls, gaps, revisions, and final MIME remain unobserved for all five-country monthly cells |
...
The current DBnomics adapter maps only `USA` in `_ISO3_TO_IFS_CC` for this design. For
`CAN/MEX/CUB/DOM`, `DBnomicsSource.get_indicator` rejects the absent IFS mapping before
`_request_json`/network (`vnfin/macro/dbnomics.py:204-229`); the public failover therefore reports a
bounded failure, not an `IndicatorSeries`. The candidate `M.{CC}.{CONCEPT}` route is future
source-design evidence only and is not current behavior. The matrix records this distinction in
each unmapped monthly row rather than treating the route template as a response.
...
### Mandatory current API audit

The current public, module, source, model, snapshot, and diagnostic surfaces were audited before
treating the matrix as a design-only disposition; no implementation change is authorized here.

#### Exact public and source signatures

The current callable boundary is:
...
MacroClient(sources=None, max_attempts: int = 3, http_get=None, timeout: float = 25.0)
MacroClient.get_indicator(country_iso3: str, indicator) -> IndicatorSeries
WorldBankMacroSource(
    http_get=None, timeout: float = 25.0, per_page: int = 20000
)
WorldBankMacroSource.get_indicator(
    country_iso3: str,
    indicator_code: str,
    start_year: int | None = None,
    end_year: int | None = None,
) -> IndicatorSeries
WorldBankMacroSource.get_canonical_indicator(
    country_iso3: str, indicator
) -> IndicatorSeries
DBnomicsSource(http_get=None, timeout: float = 25.0)
DBnomicsSource.get_indicator(country_iso3: str, indicator) -> IndicatorSeries
```

The exact current `vnfin.macro.__all__` order is:

```text
IndicatorSeries, MacroIndicator, MacroIndicatorSpec, Frequency,
canonical_unit, canonical_currency, canonical_indicator_code,
canonical_indicator_name, eligible_sources, normalize_indicator,
WorldBankMacroSource, IMFDataMapperSource, DBnomicsSource, FREDMacroSource,
MacroClient, default_macro_sources, default_macro_client, get_indicator,
client, source
```

The ten-member `MacroIndicator` enum, current imports, and these exports remain unchanged. No ranking,
batch, cohort, coverage, or source-registration symbol is added.

#### Exact model, snapshot, and diagnostics behavior
...
- `WB` means the exact World Bank route and concept shown in that row; `DBN/IMF` means the exact
  ...
  #235 dispatch.
- `NOT_PROBED` means no cell-audit response was requested for that cell: status, complete MIME,
  redirects, returned identity, count, first/last observation, null/gap pattern, revision, and
  transport result are all `NOT_OBSERVED`, not zero or empty. The four GDP rows supported only by
  the upstream ranking unit are `PARTIAL`, not full-series qualification.
...
- The fifth column of every row has exactly one cell-local `Technical behavior = ...` section and
  one `Permission = ...` section. Technical observations are not permission grants. Permission
  explicitly covers `automation`, `caller_return`, `cache/storage`, `retention/deletion`,
  `terms_version`, `terms_effective_date`, `attribution`, `commercial/derivative_use`,
  `redistribution`, `amendment`, and `revocation`, including conservative
  `NOT_RETAINED`, `NOT_CLEARED`, or `NO_NEW_GRANT`; no row inherits a legal field.
...
|---|---|---|---|---|---|
| `USA × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | Upstream #235 ranking receipt `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md` (blob `621545ca505ff6234cad32b9be7989d5e5add426`) returned countryiso3code=USA, indicator.id=NY.GDP.MKTP.CD, date=2020, value=21,375,281,000,000, unit=current US$; it is one-year ranking evidence only, with no full series count/bounds/null/gap/revision/current-lag or complete transport/terms receipt. No post-intake #235 cell-audit response was made for this cell; no auth. | Example observation exists but full count/bounds, nulls, revisions, and current lag are not retained; WB default CC BY candidate with dataset/third-party caveat; existing on-demand `IndicatorSeries`, no new body/cache; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
| `USA × GDP_GROWTH` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/NY.GDP.MKTP.KD.ZG?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · real GDP growth, not GDP level · published annual, projection flag not retained | No retained exact USA response for this cell in the cited evidence; no status/MIME/redirect/identity/session; no auth | Count/bounds/nulls/gaps/revisions not observed; WDI legal candidate only, no new redistribution claim; no #235 cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
| `USA × CPI` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/FP.CPI.TOTL?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `index` · none · CPI level, not inflation · published annual, projection flag not retained | Retained global report records USA 65 non-null observations for 1960–2024; exact country/code identity is the WDI route; no post-intake #235 cell-audit dispatch or complete status/MIME/redirect receipt; no auth | Observed count=65, first=1960, last=2024 in older retained report; current gap/revisions/terms receipt not re-established; no new cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
| `USA × INFLATION` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/FP.CPI.TOTL.ZG?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · CPI YoY inflation, not CPI level · published annual, projection flag not retained | Older report retains USA examples for 2022 and 2023; exact route identity is code-bound; no #235 status/MIME/redirect/session receipt; no auth | Example response only; complete count/bounds/null/revision/current-lag not retained; WDI terms remain candidate with caveat; no body/cache added; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
| `USA × UNEMPLOYMENT` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/SL.UEM.TOTL.ZS?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · labour-force unemployment share, bounded 0–100 · published annual, projection flag not retained | No exact per-cell USA response is retained. The older global report's broad multi-country statement does not bind country/code/status/MIME/row identity for this cell; count/bounds/null/gaps/revisions/current lag and terms receipt are not observed; no post-intake #235 cell-audit response; no auth. | Exact count/bounds/null/gaps/revisions/current lag not retained; no new legal/cache claim; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
| `USA × CPI_YOY` | DBN/IMF / IMF owns concept, DBnomics operates route / `GET https://api.db.nomics.world/v22/series/IMF/IFS/M.US.PCPI_PC_CP_A_PT?observations=1` | monthly · `%` · none · CPI change versus same month prior year, not annual WDI inflation · published monthly, projection flag not retained | Route is mechanically mapped in current code, but no response was requested for #235; returned identity/status/MIME/redirect/session/count are not observed; no auth | No date bounds/null/gap/revision receipt; IMF/DBnomics terms require exact downstream review; no raw/cache data; Technical behavior = automation=current adapter route after exact USA mapping; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response row/bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=NOT_RETAINED; commercial/derivative_use=NOT_CLEARED; redistribution=NOT_CLEARED; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
| `USA × POLICY_RATE` | DBN/IMF / IMF owns IFS, DBnomics operates route / `GET https://api.db.nomics.world/v22/series/IMF/IFS/M.US.FPOLM_PA?observations=1` | monthly · `% per annum` · none · monetary-policy-related rate only; not proven announced US policy rate · published monthly, projection flag not retained | No post-intake #235 cell-audit response; current public display override says `SBV refinancing-rate proxy`, which contradicts North American identity; no status/MIME/redirect/session; no auth | Exact US concept/authority, count/bounds/nulls/revisions, and terms not proven; no cache/body; Technical behavior = automation=current adapter route after exact USA mapping; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response row/bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=NOT_RETAINED; commercial/derivative_use=NOT_CLEARED; redistribution=NOT_CLEARED; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `SEMANTICS_GAP` |
| `USA × LENDING_RATE` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/FR.INR.LEND?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · aggregate lending rate, not policy rate · published annual, projection flag not retained | No post-intake #235 cell-audit response or returned identity/status/MIME/redirect/session; no auth | Count/bounds/null/gaps/revisions/current lag not observed; WDI legal candidate only; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
| `USA × DEPOSIT_RATE` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/FR.INR.DPST?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · aggregate deposit rate, not a tenor curve · published annual, projection flag not retained | No post-intake #235 cell-audit response or returned identity/status/MIME/redirect/session; no auth | Count/bounds/null/gaps/revisions/current lag not observed; WDI legal candidate only; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
| `USA × REAL_INTEREST_RATE` | WB / World Bank / `GET https://api.worldbank.org/v2/country/USA/indicator/FR.INR.RINR?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · real interest rate, may be negative · published annual, projection flag not retained | No post-intake #235 cell-audit response or returned identity/status/MIME/redirect/session; no auth | Count/bounds/null/gaps/revisions/current lag not observed; WDI legal candidate only; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
| `CAN × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/CAN/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | `CAN/#200`: response-backed existing path, expected country=`CAN`, code exact; historical status/MIME/redirect/session details not retained; no auth | Retained count=66; exact first/last/null/gap/revision/current-lag receipt not retained; existing WDI route/terms, no new cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PROVEN_EXISTING` |
| `CAN × GDP_GROWTH` | WB / World Bank / `GET https://api.worldbank.org/v2/country/CAN/indicator/NY.GDP.MKTP.KD.ZG?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · real GDP growth · published annual, projection flag not retained | `CAN/#200`: response-backed existing path, country/code expected exact; status/MIME/redirect/session details not retained; no auth | Retained count=65; first/last/null/gap/revision/current-lag not retained; WDI terms/cache boundary as above; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PROVEN_EXISTING` |
...
| `MEX × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/MEX/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | Upstream #235 ranking receipt `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md` (blob `621545ca505ff6234cad32b9be7989d5e5add426`) returned countryiso3code=MEX, indicator.id=NY.GDP.MKTP.CD, date=2020, value=1,121,064,767,168.8, unit=current US$; it is one-year ranking evidence only, with no full series count/bounds/null/gap/revision/current-lag or complete transport/terms receipt. No post-intake #235 cell-audit response was made for this cell; no auth. | Count/bounds/null/gaps/revisions/current lag not observed; WDI terms candidate; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
| `MEX × GDP_GROWTH` | WB / World Bank / `GET https://api.worldbank.org/v2/country/MEX/indicator/NY.GDP.MKTP.KD.ZG?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `%` · none · real GDP growth · published annual, projection flag not retained | No response/identity/status/MIME/redirect/session; no auth | Count/bounds/null/gaps/revisions/current lag not observed; WDI terms candidate; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `NOT_PROBED` |
...
| `CUB × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/CUB/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | Upstream #235 ranking receipt `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md` (blob `621545ca505ff6234cad32b9be7989d5e5add426`) returned countryiso3code=CUB, indicator.id=NY.GDP.MKTP.CD, date=2020, value=107,352,000,000, unit=current US$; it is one-year ranking evidence only, with no full series count/bounds/null/gap/revision/current-lag or complete transport/terms receipt. No post-intake #235 cell-audit response was made for this cell; no auth. | 2020 value is retained for cohort ranking, but this matrix cell's series count/bounds/null/gaps/revisions/current lag were not audited; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
...
| `DOM × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/DOM/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | Upstream #235 ranking receipt `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md` (blob `621545ca505ff6234cad32b9be7989d5e5add426`) returned countryiso3code=DOM, indicator.id=NY.GDP.MKTP.CD, date=2020, value=78,546,672,406.8291, unit=current US$; it is one-year ranking evidence only, with no full series count/bounds/null/gap/revision/current-lag or complete transport/terms receipt. No post-intake #235 cell-audit response was made for this cell; no auth. | 2020 value is retained for cohort ranking, but series count/bounds/null/gaps/revisions/current lag are not retained; WDI terms candidate; no cache/body; Technical behavior = automation=existing one-country/one-indicator runtime; caller_return=IndicatorSeries on valid result; cache/storage=off by default (no #235 cache); retention/deletion=#235 retains no response body/header/row bundle. Permission = automation=NOT_CLEARED; caller_return=NOT_CLEARED; cache/storage=NOT_CLEARED; retention/deletion=NOT_CLEARED; terms_version=NOT_RETAINED; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
...
**Matrix count:** 50 independent cells = 5 `PROVEN_EXISTING` + 6 `PARTIAL` + 1
`SEMANTICS_GAP` + 38 `NOT_PROBED`. `PARTIAL` comprises USA GDP/CPI/INFLATION plus one-year
2020 ranking evidence for MEX/CUB/DOM GDP; USA unemployment is downgraded to `NOT_PROBED` because
its older broad statement lacks an exact retained per-cell response. The count is an evidence
disposition, not a success rate or coverage assertion. The five `CAN/#200` cells remain existing
behavior and do not imply current 2026 bounds.
...
#235: this builder made no post-intake cell-audit provider dispatch. The upstream ranking observation
unit is retained separately above and does not activate this plan.

### WDI plan (not dispatched)

- Reserve exactly `logical=1`, `physical=1`, `retries=0` for this one URI:
  `GET https://api.worldbank.org/v2/country/USA;CAN;MEX;CUB;DOM/indicator/NY.GDP.MKTP.CD;NY.GDP.MKTP.KD.ZG;FP.CPI.TOTL;FP.CPI.TOTL.ZG;SL.UEM.TOTL.ZS;FR.INR.LEND;FR.INR.DPST;FR.INR.RINR?date=1960:2025&format=json&per_page=20000&source=2`.
  It requests the five frozen countries, eight WDI annual concepts, and `1960:2025`: the maximum
  logical cell envelope is `5 × 8 × 66 = 2,640` country-indicator-year positions, including nulls.
...
- Require one JSON envelope and retain the returned source identity, `page`, `pages`, `per_page`,
  and `total` exactly as observed. The response source must reconcile to requested `source=2`; the
  provider-declared total is observed, not predeclared as `2640`, and must reconcile with the
  provider's documented row/null semantics and every returned identity/key. A multi-page or
  unresolved pagination result cannot be followed under the one-physical-call reservation. Wrong
  MIME, HTML/WAF, unexpected redirect, timeout, malformed envelope, identity/source mismatch, byte
  exhaustion, or late failure invalidates the whole observation; it cannot yield a partial series.
...
## Characterization, deferred RED, and release matrix

No RED authorization is requested or implied. The following existing behavior and future behavior
are separate families.

### Existing World Bank characterization (not RED)

The current runtime's 40 public World Bank cases are the existing `5 × 8` single-country,
single-indicator calls through `WorldBankMacroSource.get_indicator`/`get_canonical_indicator`.
They characterize the current route (one country, one WDI code, current adapter query, parser,
validation, `IndicatorSeries`, failover/unit filtering, and no-network synthetic fixtures) and are
expected-green compatibility cases. They do **not** expect the future semicolon multi-country route,
`source=2` batch behavior, or a new public field. They are not a source qualification or RED
authorization.

### Future batch RED (not authorized)

After a separate API/model decision and reviewer RED authorization, a new batch seam may be tested
with synthetic offline fixtures only:

1. **World Bank batch:** one exact `source=2` multi-country/multi-indicator route for
   `USA/CAN/MEX/CUB/DOM × 8 WDI concepts`, request `date=1960:2025&format=json&per_page=20000`,
   response-backed returned source identity, observed `page/pages/per_page/total` and their
   documented semantics, maximum 2,640-position envelope, country/code identity, unit/currency/
   frequency, Jan-1 dates, ascending unique points, null/missing positions, atomic one-dispatch
   behavior, and no country-specific production branch. No test may preassert `total=2640` without
   a retained response contract.
2. **Monthly exact-dimension cases:** every future-qualified DBnomics country/concept pair, exact
   two-letter path, monthly periods, country-neutral identity, source/operator attribution, no
   SBV-label leakage, staleness warning behavior, and unsupported/unmapped countries failing before
   a malformed dispatch.
...
4. **Coverage/atomic cases:** exact full/partial/not-served/unknown distinctions, provider current
   lag, null years, revision/correction, authoritative empty versus transport failure, page/total
   mismatch, late failure, and no false partial return.
...
6. **Regression/release cases:** existing VNM/USA/CAN behavior, source ordering, unit filtering,
...
North America policy-rate result is claimed. The API/model decision for this hazard is mandatory
before #235 can close; a design PASS cannot silently accept the current label or diagnostics as a
North American policy-rate contract.
...
`472cfe6`. A design PASS would authorize only the next explicitly reviewed API/model decision, never RED,
production code, source registration, push, or closure.
...
- Reviewer-packet evidence `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md`
  (blob `621545ca505ff6234cad32b9be7989d5e5add426`) — upstream 2020/2025 GDP ranking responses,
  ranking-only and separate from the builder's future cell-audit plan.
...
- Matrix: 50 independent cells: 5 proven CAN, 6 partial (USA GDP/CPI/INFLATION plus MEX/CUB/DOM GDP),
  1 policy-rate semantics gap, and 38 not probed.
