...
anchor `acbbb82`, path `docs/research/2026-08-31-issue235-macro-cohort.md`, blob
...
This binds `route_version=v2`, `method=GET`, and no extra source parameter. The expected response is
one JSON envelope `[metadata, observations]`; metadata must reconcile `page=1`, `pages=1`,
`per_page=20000`, and `total=2640` with the 2,640 country/indicator/year positions, including
null values. Every observation must carry one of the five requested `countryiso3code` values, one
of the eight exact `indicator.id` codes, a four-digit `date`, and the provider `value`/`unit`; the
returned identity and unique `(countryiso3code, indicator.id, date)` keys must account for every
expected position. Missing/null positions stay missing. A missing row, unexpected identity, page/total
mismatch, or late failure invalidates the whole observation. This route is bound as design evidence,
not a dispatch.
...
The current public and diagnostic surfaces were audited before treating the matrix as a design-only
disposition; no implementation change is authorized here:

| Surface | Current repository behavior | Required #235 treatment |
|---|---|---|
| Public call/export | `vnfin.macro.get_indicator(country_iso3, MacroIndicator)` returns `IndicatorSeries`; the ten-member enum and no ranking/batch API remain current. | Preserve the signature/export and characterize existing behavior before any change. |
| Model/snapshot/serialization | Frozen `IndicatorSeries` carries `(date, value)` points plus `source`, `unit`, `value_unit`, `currency`, `frequency`, `projection_from_year`, `fetched_at_utc`, and `warnings`; `to_dataframe()` emits `date`, `value`, `is_projection` and metadata in `df.attrs`. | No new fields, coverage object, ranking state, or snapshot/serialization change is proposed. |
| Live source identity | `vnfin/macro/dbnomics.py:75-82,141-150` maps `USA`, while the shared `POLICY_RATE` display string names an `SBV refinancing-rate proxy`. | `USA × POLICY_RATE` stays `SEMANTICS_GAP`; that Vietnam-specific label must not be presented as a North American identity. |
| Warnings/diagnostics | `vnfin/diagnostics.py:230-249,536-582` recommends `get_indicator(..., "policy_rate")` and describes the SBV proxy. | A later API/model decision must choose country-neutral identity, display, and diagnostic text before this hazard can close. |
| Zero-capable failover | `vnfin/macro/client.py:205-228` raises `AllSourcesFailed(..., attempts=())` when no source is unit-capable, before network dispatch. | Preserve this current empty-attempts behavior as characterization until a separately authorized API decision says otherwise. |
| Cache | `vnfin/transport.py:313-324,406-410` has caching off by default; opt-in keys include URL, params, body, and headers. | Any future cache/rights decision must retain this isolation; no #235 cache is added. |

The USA policy-label/diagnostic issue is an active correctness hazard, not a generic unserved cell.
Before #235 can close, a separate API/model decision must record the country-neutral identity/display,
diagnostic warning/suggested-action behavior, and characterization-versus-RED boundary. Until then
no USA policy result may be described as the announced US policy rate, and no production or RED change
is authorized.
...
- The fifth column of every row ends with a cell-local split: technical behavior fields are not
  permission grants, and every permission field is explicit even when it is `NOT_RETAINED`,
  `NOT_CLEARED`, or `NO_NEW_GRANT`; no row inherits legal facts from the provider section.
...
| `CUB × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/CUB/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | Upstream #235 ranking receipt `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md` (blob `621545ca505ff6234cad32b9be7989d5e5add426`) returned countryiso3code=CUB, indicator.id=NY.GDP.MKTP.CD, date=2020, value=107,352,000,000, unit=current US$; it is one-year ranking evidence only, with no full series count/bounds/null/gap/revision/current-lag or complete transport/terms receipt. No post-intake #235 cell-audit response was made for this cell; no auth. | 2020 value is retained for cohort ranking, but this matrix cell's series count/bounds/null/gaps/revisions/current lag were not audited; no cache/body ; terms_version=World Bank public-licenses page observed 2026-08-31; effective_dataset_terms=NOT_RETAINED; automation=existing runtime; caller_return=IndicatorSeries; cache/storage=#235 none; retention/deletion=no body/bundle; commercial/derivative/redistribution=NO_NEW_GRANT; amendment/revocation=NOT_RETAINED Cell-local split: technical behavior = automation=existing runtime; caller_return=IndicatorSeries; cache/storage=#235 none; retention/deletion=no #235 body/bundle. Permission = terms_version=World Bank public-licenses page observed 2026-08-31; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
...
| `DOM × GDP` | WB / World Bank / `GET https://api.worldbank.org/v2/country/DOM/indicator/NY.GDP.MKTP.CD?format=json&per_page={N}&date={Y1}:{Y2}` | annual · `current US$` · USD · nominal GDP level · published annual, projection flag not retained | Upstream #235 ranking receipt `acbbb82:docs/research/2026-08-31-issue235-north-america-macro-cohort.md` (blob `621545ca505ff6234cad32b9be7989d5e5add426`) returned countryiso3code=DOM, indicator.id=NY.GDP.MKTP.CD, date=2020, value=78,546,672,406.8291, unit=current US$; it is one-year ranking evidence only, with no full series count/bounds/null/gap/revision/current-lag or complete transport/terms receipt. No post-intake #235 cell-audit response was made for this cell; no auth. | 2020 value is retained for cohort ranking, but series count/bounds/null/gaps/revisions/current lag are not retained; WDI terms candidate; no cache/body ; terms_version=World Bank public-licenses page observed 2026-08-31; effective_dataset_terms=NOT_RETAINED; automation=existing runtime; caller_return=IndicatorSeries; cache/storage=#235 none; retention/deletion=no body/bundle; commercial/derivative/redistribution=NO_NEW_GRANT; amendment/revocation=NOT_RETAINED Cell-local split: technical behavior = automation=existing runtime; caller_return=IndicatorSeries; cache/storage=#235 none; retention/deletion=no #235 body/bundle. Permission = terms_version=World Bank public-licenses page observed 2026-08-31; terms_effective_date=NOT_RETAINED; attribution=REQUIRED_IF_APPLICABLE; commercial/derivative_use=NOT_CLEARED; redistribution=NO_NEW_GRANT; amendment=NOT_RETAINED; revocation=NOT_RETAINED. | `PARTIAL` |
...
- Reserve exactly `logical=1`, `physical=1`, `retries=0` for this one URI:
  `GET https://api.worldbank.org/v2/country/USA;CAN;MEX;CUB;DOM/indicator/NY.GDP.MKTP.CD;NY.GDP.MKTP.KD.ZG;FP.CPI.TOTL;FP.CPI.TOTL.ZG;SL.UEM.TOTL.ZS;FR.INR.LEND;FR.INR.DPST;FR.INR.RINR?date=1960:2025&format=json&per_page=20000`.
  It covers the five frozen countries, eight WDI annual concepts, and `1960:2025`: the maximum
...
- Require one JSON envelope with `page=1`, `pages=1`, `per_page=20000`, `total=2640`; reconcile
  every returned `countryiso3code`, `indicator.id`, date, value/unit, and unique key against all
  2,640 expected positions. A page/total mismatch, wrong MIME, HTML/WAF, unexpected redirect,
  timeout, malformed envelope, identity mismatch, byte exhaustion, or late failure invalidates the
  whole observation; it cannot yield a partial series.
...
1. **World Bank 40-cell matrix over one exact multi-route:** `USA/CAN/MEX/CUB/DOM × 8 WDI
   concepts`, the semicolon-separated route and `date=1960:2025&per_page=20000`, returned
   country/code identity, unit/currency/frequency, Jan-1 dates, ascending unique points, null/missing
   positions, exact `page/pages/per_page/total` reconciliation, one-dispatch behavior, and no
   country-specific production branch. Fixtures may use real contract identifiers only when identity
   routing is the behavior under test; all display names, dates, and values remain synthetic.
