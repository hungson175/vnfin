...
retained. Static primary-source pages and PDFs are evidence review only; they are not candidate data
fetches.
...
| Unit | Primary evidence and exact operation boundary | Positive evidence retained | Decisive gaps and disposition |
| --- | --- | --- | --- |
...
| `OBSERVED-UAT-API` | [Observed UAT developer page](https://uat-apim-developer.csiweb.cloud/GettingStarted); host operator `UNKNOWN`; API product/discovery operation only; no production attribution | The page states that a subscription key is needed, API products are exposed through its dashboard, and autonomous-agent access requires written authorization; these are observed page statements only | No exact CSI 300 product/route/response or history coverage was identified; subscription key is not no-login; `AUTH_REQUIRED` + `IDENTITY_GAP` + `LEGAL_GAP` |
...
| Observed UAT page (operator unknown) | The page exposes API-product/dashboard context but does not name a legal owner | `csiweb.cloud` route operator is `UNKNOWN` | Observed subscription-key and written-authorization statements only; no production or no-login route is claimed |
...
Static web/document reads were bounded evidence reads from the listed primary pages/documents; they do not qualify a route. Their per-request HTTP status,
