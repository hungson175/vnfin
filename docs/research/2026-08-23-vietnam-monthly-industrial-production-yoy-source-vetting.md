...
| NSO PXWeb retained shell route attempts (one shell UI route plus three API candidates) | 4 / 4 | 0 for this retained shell cell | no data page/cursor reached | all four bounded shell attempts timed out; response status and both MIME fields are **NOT_RETAINED** | table metadata only |
| NSO PXWeb browser-rendered UI and subresource traffic | logical/physical totals **NOT_RETAINED** | **NOT_RETAINED** | **NOT_RETAINED** | browser UI rendered; browser request/subresource status, sanitized complete Content-Type, normalized media type, redirects, and retries are **NOT_RETAINED** | UI metadata only; excluded from the shell total |
...
| PXWeb table `E07.01` | [Official PXWeb table](https://pxweb.nso.gov.vn/pxweb/en/Industry/Industry/E07.01.px/), GET/UI, no login observed | UI metadata describes industrial activity and year dimensions and a unit of `%`; the table is an index table, not a proven month-keyed provider-published YoY series | Retained shell pass = one shell UI route plus three API-path candidates, `4 / 4` logical/physical attempts, zero retries; browser-rendered UI/subresource traffic is separate and `NOT_RETAINED`. No response status/MIME, rows, totals, or page contract was accepted for the timed-out shell attempts |
