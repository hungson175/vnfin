...
The future contract must set `max_concurrency = 1`, reserve every dimension before dispatch, including archive pages and documents, count
...
| Transport/document | HTML/PDF/XLS/XLSX/CSV identity; streamed attachment identity; full post-first-colon MIME; status/redirect/WAF/TLS/UA/session; archive pagination; malformed/truncated/oversized input and sanitized diagnostics | Not authorized |
...
4. bounded auth/session/UA/WAF, status/MIME/redirect/pagination, streamed attachment identity,
   archive-page/document counts, rate/concurrency/retry, compressed/decompressed-byte evidence, and
