...
baskets establish the intersection for the exact effective event. The bounded April snapshot, timed-
out July lead, and `BVH → DGC` release context do not satisfy that gate: they remain 2025 evidence only,
...
The future contract must set `max_concurrency = 1`, reserve every dimension before dispatch, including archive pages and documents, count every retry and followed redirect as a new physical operation, charge compressed bytes while streaming, and charge decompressed bytes incrementally during decoding before full materialization. Each archive entry must have a decompressed expansion ceiling and the container must have an aggregate expansion ceiling; both are enforced before bytes become a materialized document. Reconcile each dimension as
...
| Identity and revision | Exact VN30/document/revision; unique/disjoint deltas; complete-basket versus delta-only; periodic-versus-extraordinary review; no-change event; duplicate/conflicting revisions; deterministic active-revision ordering; correction/withdrawal/postponement/conflict cases | Not authorized |
...
| Transport/document | HTML/PDF/XLS/XLSX/CSV identity; streamed attachment identity; URL suffix/`Content-Disposition`/sanitized filename/normalized MIME/file magic/container agreement; full post-first-colon MIME; status/redirect/WAF/TLS/UA/session; HTTPS downgrade and host checks; redirect-loop/hop-exhaustion and final-identity-mismatch REDs; archive pagination REDs for repeated/cyclic cursor, skipped/duplicate page, changing total, overlapping identity, cross-owner next link, and late-page discard; incremental per-entry/aggregate decompression ceilings; malformed/truncated/oversized input and sanitized diagnostics | Not authorized |
...
| Diagnostics/security | UTC-aware retrieval time; finite real attempts; exact warning/error grammar; sanitized source identity; no URL/query/raw body/header/cookie/local path/provider-exception leakage | Not authorized |
| Release | Full offline suite, import/version, `docs/api.md`, `docs/units.md`, tutorial, architecture, source/skill docs, `CHANGELOG.md`, blacklist/secret/diff/path/object/clean-tree, wheel/sdist, exact remote anchor/ancestry/three paths | Not authorized |
...
1. one exact official provider route set with response/document-backed `VN30` identity, publication/
   effective dates, UTC-aware retrieval timestamp, event type, delta/complete-basket semantics,
   revision, correction, and supersession rules for every discovery, landing, attachment, and feed/API
   unit used;
...
3. written or exact published rights covering automation, caller return, cache/storage/retention,
   attribution, derivative/commercial use, redistribution/resale, amendment, and revocation for
   every route-set unit, signed by the current authorized licensor for that operation;
4. bounded auth/session/UA/WAF, status/MIME/redirect/pagination, streamed attachment identity,
   URL-suffix/filename/MIME/magic/container agreement, allowed-host and HTTPS-downgrade checks,
   archive-page/document counts, finite redirect/retry/rate/concurrency budgets, incremental
   compressed/decompressed-byte evidence, and sanitized diagnostics;
