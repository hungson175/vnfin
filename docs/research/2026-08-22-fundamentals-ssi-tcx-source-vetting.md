...
- `OK` names only the source that produced the accepted report; `NOT_SERVED` names the
  responsible non-serving source; `MISSING` and `SOURCE_ERROR` do not expose a failed-source
  trail.
