...
| Provider / page | Candidate label or metadata (retention state) | Retained type and grammar | Level and identity | Provider meaning | Disposition |
|---|---|---|---|---|---|
| VNDirect annual/quarterly archive | NOT_RETAINED — candidate: posting date/time on an archive entry | human-rendered date/time candidate; exact machine type, timezone, precision, nullability=NOT_RETAINED | VNDIRECT's own disclosure/archive entry; not arbitrary-symbol API report identity | archive posting semantics not bound to api-finfo availability | NOT_RETAINED; SEMANTICS_GAP |
| VNDirect financial-report navigation | NOT_RETAINED — candidate: Full-Year, Quarter 1-4, period-count and VND labels | human-rendered label candidate; exact schema=NOT_RETAINED | investor-relations presentation identity | period/cadence and unit context only | NOT_RETAINED; SEMANTICS_GAP |
| CafeF disclosure list | NOT_RETAINED — candidate: Thời gian gửi | human-rendered date/time candidate; exact type, timezone, precision, nullability=NOT_RETAINED | disclosure-list item; upstream sender, CafeF ingestion, and API report binding=NOT_RETAINED | semantics not defined as first public availability | NOT_RETAINED; SEMANTICS_GAP |
| CafeF document list | NOT_RETAINED — candidate: Thời gian cập nhật | human-rendered label candidate; exact machine type and semantics=NOT_RETAINED | document-list item | not bound to API report availability or revision | NOT_RETAINED; SEMANTICS_GAP |
| CafeF document/article pages | NOT_RETAINED — candidate: page or article date/time and Thời gian | human-rendered page-metadata candidate; timezone and precision=NOT_RETAINED | page/disclosure article, not response report unit | no provider definition tying it to public financial-data availability | NOT_RETAINED; SEMANTICS_GAP |
| CafeF company page | NOT_RETAINED — candidate: Cập nhật | human-rendered quote/current-snapshot candidate | quote/profile context, not statement identity | cannot substitute for report availability | NOT_RETAINED; SEMANTICS_GAP |
| CafeF report navigation | NOT_RETAINED — candidate: Năm, Quý, Theo quý, Theo năm, Lũy kế 6 tháng, Qn/YYYY, CN/YYYY | period-label candidates; timezone=N/A | fiscal/cadence identity | period selection only | NOT_RETAINED; SEMANTICS_GAP |
| CafeF document page | NOT_RETAINED — candidate: Định dạng and Tải về | file/access-affordance candidates; no availability type | document access context | access indicator only; no reuse permission | NOT_RETAINED; SEMANTICS_GAP |
| CafeF analysis-report pages | NOT_RETAINED — candidate: Ngày phát hành | page-family candidate outside the financial-statement route | analysis-report identity, not statement response identity | semantics cannot be borrowed across report families | NOT_RETAINED; SEMANTICS_GAP |

The static context pages are [VNDirect annual archive](https://www.vndirect.com.vn/danh_muc_bao_cao/thong-tin-tai-chinh/?key=bao-cao-tai-chinh-hang-nam), [VNDirect quarterly archive](https://www.vndirect.com.vn/danh_muc_bao_cao/thong-tin-tai-chinh/?key=bao-cao-tai-chinh-hang-quy), [CafeF disclosure landing](https://cafef.vn/du-lieu/cong-bo-thong-tin.chn), [CafeF announcement reports](https://cafef.vn/du-lieu/announcement-reports.chn), and [CafeF financial landing](https://cafef.vn/du-lieu/BaoCaoTaiChinh.aspx). No exact label-level snapshot/blob/locator was retained for any row, so every candidate label above is explicitly `NOT_RETAINED`; these family URLs are not reproducible label-level evidence and no label proves presentation or availability semantics.
...
### 5.1.1 Static route grammar gate

The canonical VNDirect query grammar is `&size={SIZE}&page={PAGE}`. A narrow documentation-only
static gate scans both this report and the design note: it accepts the equals assignments and rejects
any colon assignment immediately after `size` or `page` (for example, `&size:<value>` or
`&page:<value>`). This gate is lexical evidence hygiene only; it authorizes no provider request,
runtime parser, RED test, or API change.
...
### 8.3 Per-unit terms version/date

The route-unit legal ledger carries explicit unknowns rather than inheriting a provider-wide term
date. For every unit, `terms_version=NOT_RETAINED` and `terms_effective_date=NOT_RETAINED` mean
that no exact route-local version or effective date was retained; neither field is a permission or
an inferred current-term assertion.

| Unit | terms_version | terms_effective_date | Legal evidence boundary |
|---|---|---|---|
| VD-A-INC | NOT_RETAINED | NOT_RETAINED | WEB-VD-TERMS; route-local grant not retained |
| VD-A-BAL | NOT_RETAINED | NOT_RETAINED | WEB-VD-TERMS; route-local grant not retained |
| VD-A-CF | NOT_RETAINED | NOT_RETAINED | WEB-VD-TERMS; route-local grant not retained |
| VD-Q-INC | NOT_RETAINED | NOT_RETAINED | WEB-VD-TERMS; route-local grant not retained |
| VD-Q-BAL | NOT_RETAINED | NOT_RETAINED | WEB-VD-TERMS; route-local grant not retained |
| VD-Q-CF | NOT_RETAINED | NOT_RETAINED | WEB-VD-TERMS; route-local grant not retained |
| CF-A-INC | NOT_RETAINED | NOT_RETAINED | WEB-CF-GUIDE/WEB-CF-ROBOTS; route-local grant not retained |
| CF-A-BAL | NOT_RETAINED | NOT_RETAINED | WEB-CF-GUIDE/WEB-CF-ROBOTS; route-local grant not retained |
| CF-A-CF | NOT_RETAINED | NOT_RETAINED | WEB-CF-GUIDE/WEB-CF-ROBOTS; route-local grant not retained |
| CF-Q-INC | NOT_RETAINED | NOT_RETAINED | WEB-CF-GUIDE/WEB-CF-ROBOTS; route-local grant not retained |
| CF-Q-BAL | NOT_RETAINED | NOT_RETAINED | WEB-CF-GUIDE/WEB-CF-ROBOTS; route-local grant not retained |
| CF-Q-CF | NOT_RETAINED | NOT_RETAINED | WEB-CF-GUIDE/WEB-CF-ROBOTS; route-local grant not retained |
...
- VNDirect uses `&size={SIZE}&page={PAGE}`; the static gate rejects colon assignments.
- Static labels and all twelve terms version/effective-date fields are `NOT_RETAINED` without
