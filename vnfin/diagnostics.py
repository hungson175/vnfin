...
class RequestDiagnostic:
    ...
    endpoint: str
    request: dict
    status: str  # "ok" | "coverage_gap" | "partial_coverage" | "window_too_wide" | "single_source" | "unsupported_pair" | "unsupported_frequency" | "source_disabled_pending_permission" | "unknown"
...
_FUND_COVERAGE_CAPS: tuple[SourceCapability, ...] = (
    SourceCapability(
        ...
        coverage_start=None,  # disabled: no applicable runtime coverage bound
        coverage_end=None,    # disabled/not-applicable: no runtime coverage bound
        is_default=False,
        is_opt_in=False,
        is_single_source=True,
...
def explain_fund_coverage() -> RequestDiagnostic:
    """Explain that the Fmarket fund source is disabled pending permission (no network)."""
    return RequestDiagnostic(
        domain="funds",
        endpoint="fund_metadata",
        request={},
        status="source_disabled_pending_permission",
        sources=_FUND_COVERAGE_CAPS,
        notes=("SOURCE_DISABLED_PENDING_PERMISSION; no provider call.",),
        suggested_actions=(),
    )
