...
| `vsdc_parse_degraded` | `corp_actions.dividends` | Per event: a page IS a cash dividend (title/reason) but ≥1 **primary field** was unparseable — the record date, or both `ratio_pct` AND `cash_per_share` (the affected fields are left `None`). The event is surfaced, never silently dropped; an undated event is then windowed by its `pay_date`. | #163 |
