...
| Equity GICS sector | `vnfin.equities.profile(sym)` · `vnfin.equities.sectors()` · `vnfin.equities.by_sector("VNFIN"\|"Financials")` · `vnfin.equities.universe("HOSE", with_sector=True)` | `EquitySecurity` (4 sector fields) · `tuple[GicsSector]` (10) · `EquitySector` (basket members) — **derived** (clean-room) by inverting the 10 VNAllShare sector baskets; HOSE-only ~74%, unmapped HOSE + all HNX/UPCoM → `None` (`sector_partial_coverage`, never fabricated); current-snapshot |
