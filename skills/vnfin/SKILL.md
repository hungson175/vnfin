...
| Macro | `vnfin.macro.get_indicator("VNM", vnfin.macro.MacroIndicator.GDP)` — indicators: `GDP`, `GDP_GROWTH`, `CPI`, `INFLATION`, `UNEMPLOYMENT`, `CPI_YOY`, `POLICY_RATE` (monthly SBV proxy, qualified only for VNM), `LENDING_RATE` / `DEPOSIT_RATE` / `REAL_INTEREST_RATE` (annual, World Bank `FR.INR.*`). Non-VNM `POLICY_RATE` stays missing unless a separately qualified custom source is supplied. | `IndicatorSeries` |
| Fixed-income coverage | `vnfin.diagnostics.explain_fixed_income_coverage(*, country_iso3=None)` (offline) — no argument preserves the legacy explanation; VNM identifies that request, while other valid countries receive an explicit unknown/no-qualified-policy-source diagnostic with no proxy suggestion | `RequestDiagnostic` |
...
| `proxy_substitution` | `indices.world` | Asked a raw index (`^N225`/`^SSEC`/`^STI`/`^KS11`/`^CSI300`/`^HSI`) but served a **USD ETF proxy** (`EWJ`/`FXI`/`EWS`/`EWY`/`ASHR`/`EWH`) — embeds USD/local FX, NOT a faithful tracker/raw index point series. Also exposed via the structured `PriceHistory.proxy_for` field. | #193/#197 |
