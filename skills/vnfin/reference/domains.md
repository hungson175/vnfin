...
- **Gotchas:** money is **RAW VND** (unscaled). Line items keyed by opaque itemCode; codes differ corporate vs bank. Corporate `model_type`: **`1` = BALANCE, `2` = INCOME, `3` = CASHFLOW** (#198 fixed an inverted INCOME=1/BALANCE=2 mapping — see codes below); bank `101/102/103` (balance/income/cashflow). `is_bank` AUTO-detects. Corporate income headline codes: net revenue `'21001'`, gross profit `'23100'`, profit-before-tax `'23800'`, net income (total consolidated PAT) `'23003'`, net income parent `'23000'`. Corporate balance: total assets `'12700'`, total liabilities `'13000'`, owners' equity `'14000'`, current assets `'11000'`, current liabilities `'13100'`, long-term liabilities `'13300'`, cash & equivalents `'11100'`. Corporate cashflow: operating `'32000'`, investing `'33000'`, financing `'34000'`, net change `'35000'`, end-of-period `'37000'`. **Pagination returns COMPLETE periods** (#198): a wide fiscal period spanning multiple provider pages is fully assembled (a single page could previously silently truncate a period, e.g. drop owners' equity from a 142-line balance). `RATIOS` is not money (`value_unit='ratio'`, `currency=None`, `period=UNKNOWN`). Precedence: `source=` > `sources=` > default failover.
...
- **What it does:** fetches income+balance+cashflow once each via the same `get_financials` failover (NEVER `ratios`), then maps the verified **VNDirect** codes to a fixed **v1 catalog of 26 metrics** (21 raw-mapped + 5 derived ratios) — one `MetricReport` per fiscal period. Per-statement failures are non-fatal while at least one usable fiscal date exists; an all-empty `metrics()` call raises the bounded `EmptyData` message directing callers to coverage diagnostics.
- **Result:** `MetricReport(symbol, period, fiscal_date, is_bank, metrics: tuple[MetricValue], statement_sources: tuple[StatementProvenance], warnings)` — `.get(id)` (returns the `MetricValue` even when unavailable), `.to_dataframe()` (one row per metric, all 26; `df.attrs` has **no** `source` key). `MetricValue(id, value, value_unit, kind, availability, fiscal_date, inputs: tuple[MetricInput], reason, warnings)` — `value` is `None` unless `availability=='available'`; `reason` carries the stable diagnostic. `MetricCoverage(symbol, period, periods: tuple[PeriodCoverage], notes, statement_fetches: tuple[StatementProvenance] = ())` carries exactly three aggregate outcomes in income/balance/cashflow order, including when there are no periods; `PeriodCoverage(fiscal_date, is_bank, statement_provenance, per_metric, named_item_count, generic_item_count, unmapped_codes, ratio_status)`.
...
- **Gotchas:** every report carries ALL 26 metrics — gaps are `availability`, never omission. v1 maps **VNDirect only**: a statement that failed over to CafeF comes back `blocked` (`"metric map not available for source 'cafef'"`), NOT `missing`. Provenance is **per statement** (`rep.statement_sources`) — there is no single `rep.source` (income/balance/cashflow may resolve to different sources; cashflow is VNDirect-only → CafeF cashflow is `not_served`). Bank metrics use only the **#157-verified** bank codes (`net_interest_income`/`loans_to_customers`/`customer_deposits` + shared); a corporate-only id on a bank → `not_applicable` (and vice-versa). Corporate codes were re-verified in **#198** against official FPT FY2025 (PwC-audited) + VIC FY2024 (EY-audited) filings; `operating_profit` has **no verified corporate code** and returns `blocked` (`"metric 'operating_profit' has no verified code for source 'vndirect' and corporate entities"`) — never a guess and never a false `missing line item`. Derived ratios are guarded (zero/negative/non-finite denominator → `missing`, never `inf`/`NaN`). **Ratios deferred to v2** — `ratio_status` is always `not_requested`; `explain_metric("roe")` raises `VnfinError`. If no usable date exists, `metrics()` raises `EmptyData("no usable {cadence} fiscal periods for symbol '{SYMBOL}'; call explain_metric_coverage()")`; coverage returns `notes == ("no_fiscal_periods",)` plus three aggregate outcomes. `source=` wins over `sources=` including `sources=[]`; only an empty effective chain with `source is None` raises the exact empty-chain `VnfinError`. Default and explicit chains filter incapable roles before failover; malformed source names map to zero-call `custom`, and public source errors are always `recoverable source error`. `metric_catalog("bank")` → BANK+BOTH; `("corporate")`/`("non_bank")` → CORPORATE+BOTH; other string → `VnfinError`.
...
- **World/US index — separate accessor & chain (#177, extended #193):** `vnfin.indices.world(symbol="SPY", start=None, end=None, *, interval=Interval.D1, sources=None, api_key=None, http_get=None, timeout=25.0, max_attempts=3)` → `PriceHistory`, over its **own** chain `[AlphaVantageIndexSource (BYOK, primary) → StooqIndexSource (keyless ^SPX, fallback)]` (factories `default_world_index_sources()` / `default_world_index_client()`). **5 supported symbols, all served via AV in USD (US-listed ETFs):** `SPY`→SPY, `QQQ`→QQQ (direct); `^N225`→EWJ, `^SSEC`→FXI, `^STI`→EWS (**loudly-labeled USD ETF proxies** — result carries `proxy_for=<asked>` + a `proxy_substitution` warning; embed USD/local FX, not faithful trackers). Any other symbol → `InvalidData` enumerating the set. **Two caveats:** (1) the Asian proxies embed FX + are not faithful trackers (EWJ≠Nikkei225, FXI≠SSE, EWS≠STI); (2) v1 series are **PRICE-RETURN, not total-return** (no dividend reinvestment, material over 10–25y). **Cross-instrument fallback:** when the Stooq `^SPX` (`index points`, ~6000) leg is served instead of the requested instrument (AV throttled), the result carries a mechanical **`fallback_instrument_served`** warning — rebase before comparing. AV reads `ALPHAVANTAGE_API_KEY`; keyless → skipped with no network call; key redacted in errors. **Reliability:** with **no key + walled keyless fallback**, raises **`MissingKey`** (names `ALPHAVANTAGE_API_KEY` + the symbol, trail-free) — the actionable config signal; `AllSourcesFailed` is reserved for key-set-but-AV-failed. Set `ALPHAVANTAGE_API_KEY` to use world-index server-side. Provenance: `docs/sources/indices-world.md`.
...
- **Failover:** World Bank → IMF → DBnomics (all no-key). `FREDMacroSource(api_key=...)` is **BYOK-only** (`FRED_API_KEY`), excluded from the default chain. No BEA/BLS sources. **`CPI_YOY` is DBnomics-only** and resolves to a single-source **monthly** chain. The default `POLICY_RATE` DBnomics route is also monthly but is qualified only for **VNM**; the country preflight removes it before cache/transport for non-VNM requests, leaving the default chain empty. A custom source that declares `supports_country` must affirm eligibility; a hookless custom source remains eligible and is still subject to returned-identity and unit guards.
...
- **POLICY_RATE is a VNM-only proxy:** the default route is IMF/IFS `FPOLM_PA`, not the announced SBV rate — `indicator_name` discloses this (`"Policy Rate (SBV refinancing-rate proxy, IMF IFS FPOLM_PA)"`); the **canonical** code/name stay `policy_rate`/`Policy Rate`. Non-VNM default requests do not receive this SBV-labelled route. A custom source with a country hook must affirm eligibility; a hookless custom source remains an explicit caller choice and is still subject to returned-identity and unit guards. Official rate: sbv.gov.vn; CPI authority: gso.gov.vn.
