...
- **Resample (#183):** `interval` accepts an `Interval` member OR a pandas alias (`'D'/'W'/'M'/'Q'/'Y'`, case-insensitive). `D1`=daily passthrough (default). `W1/MN1/Q1/Y1` aggregate the fetched daily series **client-side** (full OHLC/period, bar = last actual trading day; volume=sum). **TRAP: `'M'`→`MN1` (MONTH), never `M1` (1 minute).** Intraday (`M1/M5/M15/M30/H1`) → `UnsupportedInterval`. Resampled result carries `resampled_from_d1` (always) + `resample_partial_period` (incomplete edge period; bars kept). Network still fetches full daily range; the win is fewer returned rows. New enum members `Interval.Q1='1Q'`, `Interval.Y1='1Y'`.
