...
**Decision:** retain the current field and make its value an opaque, response-backed provider
identity. Do not truncate it to a canonical concept suffix, and do not add a parallel model field in
this decision.
...
| Any future provider | The provider's complete stable series identity as returned and validated by that adapter | The adapter must define the exact requested/returned identity pair; a route template, concept suffix, or caller echo is not sufficient. |
...
1. Existing DBnomics serialized values, equality, `repr`, DataFrame attrs, and callers that treat
   ...
   `FPOLM_PA`.
2. Existing WDI code values remain concept codes with `country` separate. The full-identity rule is
   route-specific and does not change WDI's current model shape.
3. A future concept-only consumer must not reinterpret this field. If a canonical concept field is
...
4. Wrong country, wrong frequency, wrong concept, suffix-only identity, blank identity, or a
...
country-correct identity for USA. The current USA result is therefore a known semantic hazard, not
a compatibility promise to preserve as a North American result.
...
| USA with current `FPOLM_PA` route | Must not return an `IndicatorSeries` labelled as a North American policy rate | Deliberate semantic correction, not a promise to preserve the misleading legacy USA output. It requires later RED, code review, release notes, and CHANGELOG treatment. |
| Future response-backed USA national policy-rate route | May qualify only with its own full provider-series identity, country-correct semantics, and legal/source evidence | It must not inherit `FPOLM_PA`, the SBV label, or another country's identity. |

For the future public `vnfin.macro.get_indicator("USA", MacroIndicator.POLICY_RATE)` path, the
selected fail-closed carrier is the existing `AllSourcesFailed` zero-capability form when no
country-correct source is eligible (`attempts=()`). The existing source-level carrier remains
`InvalidData` for a direct current-route identity/semantic rejection; no new exception class or
unsanitized provider text is introduced. Exact pre-network routing and message assertions belong to
the later RED/code packet and are not implemented here.

This boundary does not authorize changing the current runtime now. Until a later RED/code review,
the source-design disposition remains `SEMANTICS_GAP`, and the new chain remains empty.
...
The exact future diagnostic rule is:

1. Keep the existing zero-argument diagnostic behavior as the legacy VNM proxy description; it is
   not a North America capability statement.
2. Before any country-aware diagnostic is implemented, the API/model decision must add an explicit
   optional `country_iso3` keyword or an equivalent versioned diagnostic surface. For `VNM`, retain
   the proxy disclosure and suggestion. For `USA`, return the existing `RequestDiagnostic` carrier
   with status `unknown`, a fixed sanitized note that the policy-rate identity is not qualified for
   USA, and no suggested action that names `get_indicator(..., 'policy_rate')` or the SBV. No new
   diagnostic model field is needed.
3. A future exact USA policy-rate result may use a country-correct neutral display name and its full
   provider-series `indicator_code`; it must not inherit the SBV proxy label or warning.

This explicitly preserves unrelated public diagnostics compatibility without preserving the USA
SBV-specific label as valid identity. The optional country-aware diagnostic signature, exact note,
and pre-network fail-closed behavior require RED authorization and implementation review; none is
...
| USA public preflight | USA `POLICY_RATE` with current DBnomics route is rejected before cache/network or yields zero-attempt `AllSourcesFailed` through public failover | No USA SBV-labelled result |
| Direct source guard | Direct current-route USA call fails with existing typed `InvalidData` before provider dispatch | No raw provider message or row |
...
| Diagnostic | VNM retains proxy suggestion; USA-aware diagnostic has fixed sanitized `unknown` status/note and no SBV or generic policy-rate suggestion | No country leakage |
...
for the next review. The final backlog mirror must bind its exact blob, the clean base
`472cfe6d42ba43ab535a2ff676220896d5aaaacd`, source/design PASS `429870c`, reviewer report
`reviews/review-202608312046-issue235-final-design-approval.md`, and the next exact verdict.

## Bottom summary

- Decision D1: preserve opaque, full provider-series `indicator_code`; DBnomics keeps `M.US.FPOLM_PA`.
- WDI keeps its existing concept-code plus separate response-backed country identity.
- Decision D2: USA `POLICY_RATE` remains `SEMANTICS_GAP`; do not present `FPOLM_PA` as US policy rate.
- The SBV proxy label remains VNM legacy disclosure only, not blanket USA compatibility.
- USA diagnostic behavior must be country-aware and fail closed without SBV leakage.
- No model/export/source/probe/RED/code change is made by this packet.
- Next gate is exact API/model review, then separate RED authorization if approved.
- Need from reviewer: exact API/model decision verdict; no Boss decision is required now.
